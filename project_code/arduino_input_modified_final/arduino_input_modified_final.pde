...
//  Arduino.list()[8]
  ...
  kick = minim.loadSample( "1up.wav", // filename
                            512      // buffer size
                         );
    
      // load SD.wav from the data folder
  snare = minim.loadSample("powerup.wav", 512);
  if ( snare == null ) println("Didn't get snare!");

  coin = minim.loadSample("pause.wav", 512);
...
void draw() {
  ...
  analogvalue=arduino.analogRead(3);//channel is 0
  
 float analogvalue2=arduino.analogRead(4);//channel is 0

 float analogvalue5=arduino.analogRead(5);//channel is 0
  ...
  text(analogvalue2,100,200);
  text(analogvalue5,200,200);
text(millis(),200,100);
